...
- 状态：**Task 0–14 代码、跨栈观测、CI 与文档已实施并完成本地回归和真实 embedding 测量；Task 14 的真实 LLM/百度验收等待调用与数据处理授权，保持 not_run，不宣称线上闭环验收通过。**
...
- 批次 A、B、C 已获执行批准。代码按本文逐任务红→绿→回归→提交，禁止跳过测试、降低原 13 项门禁或重新引入 itinerary/nearby/crowd/review/ticket crawler。
...
- 最终本地验收：Python 298 passed / 1 skipped、Java 28 passed、Web 4 passed + build；112-case / 21 门禁通过，真实 embedding 28 cases 已测。上一条为历史检查点，最新范围与真实服务未运行原因见 [最终记录](2026-09-04-batch-d-final-verification.md)。
