...
from app.evidence.evidence_decision_report import AdoptionLevel, ClaimDecision


def claim_family_for_type(claim_type: str) -> str:
    return importlib.import_module("app.evidence.claim_family_registry").claim_family_for_type(
        claim_type
    )


def evidence_strength_for_claim(*args, **kwargs):
    return importlib.import_module("app.evidence.search_snippet_policy").evidence_strength_for_claim(
        *args, **kwargs
    )
...
def _structured_adopted_value(decision: ClaimDecision, evidence: list | None) -> str | None:
    ...
    if family == "ticket_booking" or ct in {
        ...
        preferred_ticket_facts = importlib.import_module(
            "app.evidence.ticket_price_audit"
        ).preferred_ticket_facts
