...
from app.evidence.claim_adoption_policy import ClaimAdoptionPolicy
from app.evidence.conflict_resolver import EvidenceConflictResolver
from app.evidence.coverage_checker import EvidenceCoverageChecker
from app.evidence.coverage_report import CoverageReport
from app.evidence.evidence_decision_report import (
    EvidenceDecisionReport,
    RejectedEvidence,
    SourceRanking,
)
from app.evidence.evidence_scorer import EvidenceScorer

TravelAgentState = Any


def _claim_policy_registry():
    return importlib.import_module("app.evidence.claim_policy_registry")


def _response_contract_class():
    return importlib.import_module("app.composition.response_contract").ResponseContract
...
class EvidenceEvaluator:
    def __init__(self) -> None:
        ...
        self.gap_planner = importlib.import_module(
            "app.planning.evidence_gap_planner"
        ).EvidenceGapPlanner()
    ...
    def evaluate(self, state: TravelAgentState, *, target_label: str) -> EvidenceDecisionReport:
        claim_registry = _claim_policy_registry()
        contract = state.response_contract or _response_contract_class()()
        claims = [claim_registry.enrich_claim_requirement(c) for c in contract.claim_requirements]
        if not claims:
            claims = [claim_registry.enrich_claim_requirement(c) for c in self._default_claims(state)]
        ...
        for claim in claims:
            policy = claim_registry.resolve_policy(claim)
            ...
            decision = importlib.import_module(
                "app.evidence.claim_decision_enrichment"
            ).enrich_claim_decision(decision, evidence=state.evidence)
    ...
    def _build_coverage_report(self, contract: ResponseContract, decisions) -> CoverageReport:
        from app.evidence.coverage_report import CoverageItem
    ...
    def _default_claims(state: TravelAgentState):
        ClaimRequirement = importlib.import_module(
            "app.composition.response_contract"
        ).ClaimRequirement
        needs = []
