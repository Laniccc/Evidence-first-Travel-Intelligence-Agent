...
def resolve_nearby_need(need: str | None) -> str | None:
    return _information_need_aliases().resolve_nearby_need(need)


def task_requires_mandatory_poi_anchor(state: Any, task: Any) -> bool:
    return importlib.import_module(
        "app.planning.s5_poi_anchor_policy"
    ).task_requires_mandatory_poi_anchor(state, task)
