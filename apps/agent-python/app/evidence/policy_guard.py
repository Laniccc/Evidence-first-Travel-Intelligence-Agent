...
from app.evidence.coverage_checker import EvidenceCoverageChecker
...
AgentAction = importlib.import_module("app.execution.actions").AgentAction
AgentActionType = importlib.import_module("app.execution.actions").AgentActionType
PolicyGuard = importlib.import_module("app.governance.policy_guard").PolicyGuard
StateNodePolicy = importlib.import_module("app.orchestration.state_policy").StateNodePolicy
EvidencePolicy = importlib.import_module("app.evidence.evidence_policy").EvidencePolicy
AnswerMode = importlib.import_module("app.understanding.semantic_frame_model").AnswerMode
ToolWhitelist = Any
TravelAgentState = Any

def comparison_max_tool_calls() -> int:
    return importlib.import_module("app.planning.comparison_helpers").comparison_max_tool_calls()

def is_comparison_mode(state: Any) -> bool:
    return importlib.import_module("app.planning.comparison_helpers").is_comparison_mode(state)

def location_usage_allowed(state: Any) -> bool:
    return importlib.import_module("app.planning.tool_whitelist_builder").location_usage_allowed(state)
...
def is_mcp_policy_placeholder(name: str) -> bool:
    return _module("app.integrations.mcp.adapter_status").is_mcp_policy_placeholder(name)


def get_mcp_client_manager():
    return _module("app.integrations.mcp.client_manager").get_mcp_client_manager()


def _tool_specs_module():
    return _module("app.integrations.mcp.tool_specs")


def _tool_name_resolver():
    return _module("app.tools.tool_name_resolver")


def is_mcp_policy_tool(tool: str) -> bool:
    return _tool_name_resolver().is_mcp_policy_tool(tool)


def resolve_tool_name(tool: str) -> str:
    return _tool_name_resolver().resolve_tool_name(tool)


def _retrieval_ledger():
    return _module("app.execution.retrieval_attempt_ledger")
...
class EvidencePolicyGuard(PolicyGuard):
    ...
    def _validate_tool_call(
        ...
        if tool == "baidu_place_search_mcp":
            ticket_lookup_policy = importlib.import_module("app.planning.ticket_lookup_policy")

            if not ticket_lookup_policy.baidu_place_search_allowed_for_ticket(state):
                raise ValueError(
        ...
        if is_ticket_provider_tool(resolved):
            ...
            if not ticket_lookup_policy.ticket_platform_tool_allowed(state, resolved):
                if state.current_evidence_gap_request and state.current_evidence_gap_request.claim_type == "ticket_price":
                    ticket_lookup_policy.force_ticket_platform_phase(state)
                if not ticket_lookup_policy.ticket_platform_tool_allowed(state, resolved):
                    raise ValueError(
        ...
        if is_mcp_policy_tool(tool):
            mcp_policy_specs = _tool_specs_module().MCP_POLICY_SPECS
    ...
    def _validate_subagent_call(
        ...
        ORCHESTRATOR_SUBAGENT_NAMES = _module(
            "app.orchestration.s5_subagent_registry"
        ).ORCHESTRATOR_SUBAGENT_NAMES
        SearchTask = _module("app.planning.search_task").SearchTask
        target = action.target or ""
        blocks_subagent_until_poi_anchor = _module(
            "app.planning.s5_poi_anchor_policy"
        ...
        if target in {"keyword_search_agent", "fact_search_agent"}:
            KeywordSearchAgent = _module("app.execution.keyword_search_agent").KeywordSearchAgent
            ...
            if not tool:
                if target == "fact_search_agent":
                    FactSearchAgent = _module("app.execution.fact_search_agent").FactSearchAgent
        ...
        if target == "entity_resolution_agent":
            entity_resolution_allowed_for_lookup = _module(
                "app.orchestration.lookup_entity_resolution_policy"
    ...
    def _validate_finish(
        ...
        retrieval_complete = ledger.retrieval_complete
        sync_ledger_to_state = ledger.sync_ledger_to_state
    ...
    def _unconfigured_or_untried_tools(
        ...
        for need in needs:
            if need not in _HARD_FACT_NEEDS:
                continue
            profile = _tool_specs_module().NEED_TOOL_PROFILES.get(need, [])
            for tool in profile:
                resolved = resolve_tool_name(tool)
    ...
    def _validate_tool_argument_requirements(tool: str, args: dict, state: TravelAgentState) -> None:
        ...
        if resolved == "official_source_discovery_mcp":
            ticket_lookup_helpers = _ticket_lookup_helpers()
            collect_ticket_search_urls = ticket_lookup_helpers.collect_ticket_search_urls
            has_ticket_url_inputs = ticket_lookup_helpers.has_ticket_url_inputs
            ...
            if not urls and not hits and not has_ticket_url_inputs(state):
                ...
                record_skip = _retrieval_ledger().record_skip
        ...
        if resolved in {"official_page_reader_mcp", "browser_mcp"}:
            ...
            if not url and not urls:
                collect_ticket_search_urls = _ticket_lookup_helpers().collect_ticket_search_urls
