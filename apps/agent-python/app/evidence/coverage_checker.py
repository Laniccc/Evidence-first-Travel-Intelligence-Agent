...
from app.evidence.official_source_judgement import best_official_support, parse_candidate_from_evidence

ClaimRequirement = Any
ResponseContract = Any
ToolTrace = Any


def is_nearby_need(need: str | None) -> bool:
    return importlib.import_module("app.understanding.information_need_aliases").is_nearby_need(
        need
    )


def _nearby_policy():
    return importlib.import_module("app.evidence.nearby_recommendation_policy")


def claim_aliases_for_need(need: str):
    return _nearby_policy().claim_aliases_for_need(need)


def is_nearby_information_need(need: str | None) -> bool:
    return _nearby_policy().is_nearby_information_need(need)


def place_candidates_is_nearby_recommendation(claim) -> bool:
    return _nearby_policy().place_candidates_is_nearby_recommendation(claim)


def _ticket_lookup_helpers():
    return importlib.import_module("app.planning.ticket_lookup_helpers")


def is_ticket_price_noise_evidence(*args, **kwargs) -> bool:
    return _ticket_lookup_helpers().is_ticket_price_noise_evidence(*args, **kwargs)


def ticket_platform_candidate_quality(*args, **kwargs):
    return _ticket_lookup_helpers().ticket_platform_candidate_quality(*args, **kwargs)


def resolve_tool_name(tool_name: str) -> str:
    return importlib.import_module("app.tools.tool_name_resolver").resolve_tool_name(tool_name)
...
class EvidenceCoverageChecker:
    ...
    def _evaluate_claim(
        ...
        if req.priority == "required" and req.claim_type == "ticket_price" and best_quality != "strong":
            ...
            classify_elevation_text = importlib.import_module(
                "app.evidence.peak_elevation_extraction"
            ).classify_elevation_text
