...
def _claim_search_planner():
    return importlib.import_module("app.planning.claim_search_planner").ClaimSearchPlanner


def _signal_utils():
    return importlib.import_module("app.evidence.evidence_signal_utils")


def _search_task_class():
    return importlib.import_module("app.planning.search_task").SearchTask
...
class EvidenceContradictionDecomposerAgent:
    ...
    async def run(self, state: Any, arguments: dict | None = None) -> dict:
        args = arguments or {}
        claim_search_planner = _claim_search_planner()
        need = str(args.get("target_need") or "").strip() or claim_search_planner.primary_information_need(state) or "ticket_price"
        if not _signal_utils().multi_value_signal_for_need(state, need):
            return {
                "decompositions": [],
                "follow_up_search_tasks": [],
    ...
    def _build_context(self, state: Any, need: str) -> dict:
        claim_search_planner = _claim_search_planner()
        ...
        return {
            ...
            "entities": (claim_search_planner.planning_context(state)).get("entities") or {},
            "evidence_highlights": claim_search_planner.evidence_highlights(state),
            "detected_conflicts": relevant_conflicts or conflicts,
            "tried_search_queries": sorted(claim_search_planner.tried_from_traces(state)),
        }
    ...
    def _tasks_from_payload(self, bucket: list, ctx: dict) -> list[Any]:
