...
from app.evidence.evidence_decision_report import EvidenceConflict


def parse_candidate_from_evidence(evidence: Evidence):
    return importlib.import_module(
        "app.evidence.official_source_judgement"
    ).parse_candidate_from_evidence(evidence)


def source_class_priority(source_class: str | None) -> int:
    return importlib.import_module(
        "app.evidence.official_source_judgement"
    ).source_class_priority(source_class)


def evidence_has_main_ticket_scope(evidence: Evidence) -> bool:
    return importlib.import_module(
        "app.evidence.ticket_price_audit"
    ).evidence_has_main_ticket_scope(evidence)
