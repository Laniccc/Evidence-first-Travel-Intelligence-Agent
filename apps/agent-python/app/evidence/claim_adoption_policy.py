...
from app.evidence.evidence_scorer import EvidenceScore
from app.evidence.evidence_model import ClaimType, SourceType
from app.evidence.evidence_decision_report import ClaimDecision, EvidenceConflict, RejectedEvidence

ClaimPolicyView = Any
IntentStrategy = Any


def apply_intent_s7_policy(*args, **kwargs):
    return importlib.import_module("app.evidence.intent_s7_policy").apply_intent_s7_policy(
        *args, **kwargs
    )


def best_official_support(*args, **kwargs):
    return importlib.import_module("app.evidence.official_source_judgement").best_official_support(
        *args, **kwargs
    )


def _ticket_claim_types() -> frozenset[str]:
    return importlib.import_module("app.orchestration.fact_lookup_policy")._TICKET_CLAIM_TYPES


def _hard_fact_sensitivity():
    return importlib.import_module("app.understanding.intent_profile").EvidenceSensitivity.HARD_FACT
...
class ClaimAdoptionPolicy:
    ...
    def decide(
        ...
        *,
        ...
        if intent_strategy and intent_strategy.evidence_sensitivity == _hard_fact_sensitivity():
    ...
    def _coverage_quality(
        ...
        *,
        ...
        if evidence and policy.claim_type in {
            "opening_hours",
    ...
    def _ticket_price_adoption(
        ...
        *,
        ...
        extract_ticket_price_from_evidence = importlib.import_module(
        ...
        evidence_strength_for_claim = importlib.import_module(
            "app.evidence.search_snippet_policy"
        ).evidence_strength_for_claim
    ...
    def _opening_hours_adoption(
        ...
        *,
        ...
        extract_opening_hours_from_evidence = importlib.import_module(
            "app.evidence.opening_hours_extractor"
        ).extract_opening_hours_from_evidence
        evidence_strength_for_claim = importlib.import_module(
            "app.evidence.search_snippet_policy"
        ).evidence_strength_for_claim
        from app.evidence.evidence_model import Evidence
