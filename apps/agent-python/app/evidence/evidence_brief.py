...
from app.evidence.evidence_brief_model import CuratedClaimRow, EvidenceBrief
...
def build_evidence_brief(state: TravelAgentState, target_label: str) -> EvidenceBrief:
    if state.evidence_decision_report:
        ...
        helpers = _comparison_helpers()
        if helpers.is_comparison_mode(state):
            brief = helpers.enrich_comparison_brief(state, brief, target_label)
        return brief
