...
ClaimPolicyView = Any
OfficialSupportSummary = Any

_claim_policy_registry = importlib.import_module("app.evidence.claim_policy_registry")
GEO_ONLY_CLAIMS = _claim_policy_registry.GEO_ONLY_CLAIMS
REVIEW_EXPERIENCE_CLAIMS = _claim_policy_registry.REVIEW_EXPERIENCE_CLAIMS
SOURCE_RELIABILITY = _claim_policy_registry.SOURCE_RELIABILITY
source_type_key = _claim_policy_registry.source_type_key

_ticket_price_audit = importlib.import_module("app.evidence.ticket_price_audit")
PLATFORM_SOURCE_CLASSES = _ticket_price_audit.PLATFORM_SOURCE_CLASSES
TICKET_CLAIMS = _ticket_price_audit.TICKET_CLAIMS
evidence_ticket_blob = _ticket_price_audit.evidence_ticket_blob
is_platform_addon_for_claim = _ticket_price_audit.is_platform_addon_for_claim


def is_search_miss_value(value: str) -> bool:
    return importlib.import_module("app.planning.claim_search_planner").is_search_miss_value(value)


def claim_aliases_for_need(need: str):
    return importlib.import_module("app.evidence.nearby_recommendation_policy").claim_aliases_for_need(
        need
    )
...
def place_candidates_is_nearby_recommendation(claim: Claim) -> bool:
    return importlib.import_module(
        "app.evidence.nearby_recommendation_policy"
    ).place_candidates_is_nearby_recommendation(claim)


def _official_source_judgement():
    return importlib.import_module("app.evidence.official_source_judgement")


def best_official_support(*args, **kwargs):
    return _official_source_judgement().best_official_support(*args, **kwargs)


def official_source_reliability(*args, **kwargs):
    return _official_source_judgement().official_source_reliability(*args, **kwargs)


def parse_candidate_from_evidence(*args, **kwargs):
    return _official_source_judgement().parse_candidate_from_evidence(*args, **kwargs)
...
def _evidence_has_extractable_ticket_fact(ev: Evidence, claim_type: str, claim: Claim) -> bool:
    _source_type_label = importlib.import_module(
        "app.evidence.search_snippet_policy"
    )._source_type_label
    ticket_price_extractor = importlib.import_module(
        "app.evidence.ticket_price_extractor"
    )
    extract_ticket_price_from_evidence = ticket_price_extractor.extract_ticket_price_from_evidence
    extract_ticket_price_from_text = ticket_price_extractor.extract_ticket_price_from_text
