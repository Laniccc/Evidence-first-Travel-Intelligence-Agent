...
from app.integrations.catalog.place_catalog import get_place_catalog
from app.integrations.storage.place_cache import PlaceCache

logger = logging.getLogger(__name__)

_place_entity_module = importlib.import_module("app.understanding.place_entity_extractor")
GEO_CITY_ALIASES = _place_entity_module.GEO_CITY_ALIASES
LLMPlaceEntityExtractor = _place_entity_module.LLMPlaceEntityExtractor
PlaceMention = Any

_place_candidate_module = importlib.import_module("app.understanding.place_candidate")
PlaceCandidate = _place_candidate_module.PlaceCandidate
PlaceResolutionSource = _place_candidate_module.PlaceResolutionSource
...
class RealPlacesResolver(BasePlaceResolver):
    ...
    async def resolve(
        ...
        if not settings.enable_real_places:
            return None
        try:
            RealPlacesTool = importlib.import_module("app.tools.real.places_tool").RealPlacesTool
            tool = RealPlacesTool()
