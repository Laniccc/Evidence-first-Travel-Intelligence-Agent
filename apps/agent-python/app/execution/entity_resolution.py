...
def _s5_registry_module():
    return _module("app.orchestration.s5_subagent_registry")


def _execution_module(name: str):
    return _module("app.execution." + name)


def _delegated_mcp_runner():
    return _module("app.integrations.mcp.delegated_runner")


def _profile():
    return _s5_registry_module().S5_SUBAGENT_PROFILES["entity_resolution_agent"]


def _nearby_claims_for_retrieval(state: Any) -> list[str]:
    return _module("app.understanding.information_need_aliases").nearby_claims_for_retrieval(state)


def _place_disambiguation_guard():
    return _module("app.orchestration.place_disambiguation")
...
class EntityResolutionAgent:
    ...
    @staticmethod
    def _task_from_arguments(arguments: dict, state: Any) -> SearchTask:
        raw = {
    ...
    @staticmethod
    def _all_nearby_claims_after_anchor(state: Any) -> list[str]:
        return _nearby_claims_for_retrieval(state)
    ...
    async def run(
        ...
        delegated_mcp_runner = _delegated_mcp_runner()
        pick_tool_from_priority = delegated_mcp_runner.pick_tool_from_priority
        run_delegated_mcp = delegated_mcp_runner.run_delegated_mcp
        guard = _place_disambiguation_guard()
        ...
        ambiguous = guard.detect_ambiguous_candidates(working_evidence)
        if ambiguous:
            guard.mark_disambiguation_pending(state, ambiguous)
            resolution_status = "ambiguous"
            for candidate in ambiguous:
                region = guard.candidate_baidu_region(candidate)
                if not region:
                    continue
        ...
        if guard.try_resolve_disambiguation(state):
            guard.clear_disambiguation_pending(state)
            resolution_status = "resolved"
        else:
            candidates = guard.extract_place_candidates(working_evidence)
            if candidates and not parser_ambiguous(candidates):
                guard.apply_unique_candidate(state, candidates[0])
                resolution_status = "resolved"
        ...
        for nearby_claim in nearby_claims:
            try:
                run_nearby_retrieval_after_anchor = _execution_module(
                    "nearby_retrieval_runner"
                ).run_nearby_retrieval_after_anchor
                run_nearby_enrichment_after_retrieval = _execution_module(
                    "nearby_enrichment_runner"
                ).run_nearby_enrichment_after_retrieval
        ...
        return {
            ...
            "place_candidates": guard.extract_place_candidates(working_evidence),
