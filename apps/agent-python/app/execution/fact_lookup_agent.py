...
from app.execution.fact_lookup_phase_runner import run_lookup_phase
from app.planning.lookup_research_chain import LookupPhase, LookupQueryObjective, SourceFamily
...
def _fact_lookup_policy():
    return _module("app.orchestration.fact_lookup_policy")


def _lookup_research_chain():
    return _module("app.orchestration.lookup_research_chain")


def is_fact_lookup_task(state: Any) -> bool:
    return _fact_lookup_policy().is_fact_lookup_task(state)


def primary_fact_need_from_state(state: Any) -> str:
    return _fact_lookup_policy().primary_fact_need_from_state(state)


def ensure_lookup_chain_initialized(state: Any) -> None:
    _lookup_research_chain().ensure_lookup_chain_initialized(state)


def is_duplicate_lookup_attempt(state: Any, signature: str) -> bool:
    return _lookup_research_chain().is_duplicate_lookup_attempt(state, signature)


def lookup_attempt_signature(**kwargs) -> str:
    return _lookup_research_chain().lookup_attempt_signature(**kwargs)


def next_recommended_phase(state: Any) -> str | None:
    return _lookup_research_chain().next_recommended_phase(state)


def record_lookup_attempt(state: Any, signature: str) -> None:
    _lookup_research_chain().record_lookup_attempt(state, signature)


def source_families_for_phase(phase: str, claim_target: str) -> list[str]:
    return _lookup_research_chain().source_families_for_phase(phase, claim_target)
