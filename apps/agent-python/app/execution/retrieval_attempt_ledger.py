...
def _primary_fact_need_from_state(state: Any) -> str | None:
...
def _structured(state: Any) -> dict:
...
def get_ledger(state: Any, claim_type: str | None = None) -> RetrievalAttemptLedger:
    claim = claim_type or _primary_fact_need_from_state(state) or "general_fact"
...
def save_ledger(state: Any, ledger: RetrievalAttemptLedger) -> None:
    structured = _structured(state)
...
def _hydrate_from_state(ledger: RetrievalAttemptLedger, state: Any) -> None:
...
def record_official_discovery_skipped(state: Any, reason: str) -> None:
    record_skip(state, "official_source", reason, claim_type="ticket_price")


def record_official_page_reader_skipped(state: Any, reason: str, *, claim_type: str | None = None) -> None:
    record_skip(state, "official_page_reader", reason, claim_type=claim_type)


def retrieval_complete(state: Any, claim_type: str | None = None) -> bool:
    ledger = get_ledger(state, claim_type)
    _hydrate_from_state(ledger, state)
...
def sync_ledger_to_state(state: Any, claim_type: str | None = None) -> dict[str, Any]:
    ledger = get_ledger(state, claim_type)
    _hydrate_from_state(ledger, state)
    save_ledger(state, ledger)
