...
from app.integrations.mcp.tool_specs import NEED_TOOL_PROFILES
...
def catalog_entry(*args, **kwargs):
    return _module("app.planning.agent_tool_catalog").catalog_entry(*args, **kwargs)
...
def build_comparison_search_query(*args, **kwargs) -> str:
    return _module("app.planning.comparison_helpers").build_comparison_search_query(*args, **kwargs)


def comparison_search_anchors(*args, **kwargs) -> list[str]:
    return _module("app.planning.comparison_helpers").comparison_search_anchors(*args, **kwargs)
...
def enrich_mcp_tool_arguments(*args, **kwargs) -> dict:
    return _module("app.integrations.mcp.tool_arguments").enrich_mcp_tool_arguments(*args, **kwargs)


def normalize_need(need: str) -> str:
    return _module("app.understanding.information_need_aliases").normalize_need(need)


def _claim_search_planner():
    return ClaimSearchPlanner


def _mcp_tool_invocation_ready(*args, **kwargs) -> bool:
    return _module("app.integrations.mcp.tool_arguments").mcp_tool_invocation_ready(*args, **kwargs)


def _select_tool_for_subagent(*args, **kwargs):
    return _module("app.execution.s5_diversified_tool_selector").select_tool_for_subagent(*args, **kwargs)


def _record_tool_attempt(*args, **kwargs) -> None:
    _module("app.execution.s5_tool_attempt_ledger").record_tool_attempt(*args, **kwargs)
...
class KeywordSearchAgent:
    ...
    def _preferred_tool_is_usable(
        ...
        *,
        ...
        if state is not None:
            ...
            if not _mcp_tool_invocation_ready(
    ...
    def apply_diversified_tool_selection(
        ...
        *,
        ...
        if cls._preferred_tool_is_usable(task, whitelist, state=state):
            return task
        selection = _select_tool_for_subagent(
            state,
            task,
            whitelist,
            subagent=subagent,
    ...
    def pick_tool(
        ...
        *,
        ...
        # Default delegated web lookup: search_mcp 鈫?open-webSearch HTTP
    ...
    async def run(
        ...
        raw = {
            ...
            or _claim_search_planner().primary_information_need(state)
            or "unknown",
