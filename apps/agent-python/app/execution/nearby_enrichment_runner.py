...
def enrichment_enabled_for_category(nearby_claim: str) -> bool:
    return _module("app.evidence.nearby_category_registry").enrichment_enabled_for_category(nearby_claim)


def enrichment_tools_for_category(nearby_claim: str) -> list[str]:
    return _module("app.evidence.nearby_category_registry").enrichment_tools_for_category(nearby_claim)


def enrichment_top_n_for_category(nearby_claim: str) -> int:
    return _module("app.evidence.nearby_category_registry").enrichment_top_n_for_category(nearby_claim)


def review_enrichment_top_n_for_category(nearby_claim: str) -> int:
    return _module("app.evidence.nearby_category_registry").review_enrichment_top_n_for_category(nearby_claim)


def enrichment_candidates_from_evidence(*args, **kwargs):
    return _module("app.evidence.nearby_enrichment_policy").enrichment_candidates_from_evidence(*args, **kwargs)


def requires_nearby_reputation_signal(state: Any) -> bool:
    return _module("app.evidence.nearby_enrichment_policy").requires_nearby_reputation_signal(state)


def tag_enrichment_claims(*args, **kwargs) -> None:
    _module("app.evidence.nearby_enrichment_policy").tag_enrichment_claims(*args, **kwargs)
