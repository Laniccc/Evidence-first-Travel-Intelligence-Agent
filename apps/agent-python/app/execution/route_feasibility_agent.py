...
def _profile():
    """Bridge to the S5 registry until Task 20N moves that composition surface."""
    return importlib.import_module("app.orchestration.s5_subagent_registry").S5_SUBAGENT_PROFILES[
...
class RouteFeasibilityAgent:
    ...
    def _task_from_arguments(arguments: dict, state: TravelAgentState) -> SearchTask:
        ...
        try:
            parsed_origin, parsed_dest = importlib.import_module(
                "app.integrations.mcp.tool_arguments"
            )._route_endpoints_from_text(state.raw_user_query)
        except Exception:
    ...
    async def run(
        ...
        return {
            ...
            "claim_target": task.claim_target or _claim_search_planner().primary_information_need(state),
