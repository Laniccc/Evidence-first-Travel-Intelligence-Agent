...
class S5ToolAttemptLedger(BaseModel):
    ...
    def sync_from_tool_traces(self, state: Any) -> None:
...
def get_ledger(state: Any) -> S5ToolAttemptLedger:
...
def save_ledger(state: Any, ledger: S5ToolAttemptLedger) -> None:
    structured = dict(state.structured_result or {})
...
def record_tool_attempt(
    ...
    *,
    ...
    try:
        from app.execution.retrieval_attempt_ledger import record_tool_attempt as record_retrieval_attempt
