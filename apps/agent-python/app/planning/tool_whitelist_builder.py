...
from app.understanding.semantic_frame_model import AnswerMode, DecisionType, QueryScope, SemanticFrame
...
from .s5_information_domain import S5DomainPlan
from .s5_information_domain_registry import placeholder_tool_names
from .tool_whitelist_model import ToolDescriptor, ToolWhitelist
...
def _evidence_policy_class():
    return _module("app.evidence.evidence_policy").EvidencePolicy


def _capability_registry_class():
    return _module("app.tools.capability_registry").CapabilityRegistry
...
def _is_mcp_policy_tool(tool_name: str) -> bool:
    return _module("app.tools.tool_name_resolver").is_mcp_policy_tool(tool_name)


def _use_java_tool_gateway() -> bool:
    return _module("app.tools.gateway_config").use_java_tool_gateway()


def _tools_mcp_module(module_name: str):
    return _module("app.tools." + "mcp." + module_name)


def _mcp_policy_specs() -> dict:
    return _tools_mcp_module("tool_specs").MCP_POLICY_SPECS


def _need_tool_profiles() -> dict:
    return _tools_mcp_module("tool_specs").NEED_TOOL_PROFILES


def _is_mcp_policy_placeholder(tool_name: str) -> bool:
    return _tools_mcp_module("adapter_status").is_mcp_policy_placeholder(tool_name)


def _mcp_policy_stub_reason(tool_name: str) -> str | None:
    return _tools_mcp_module("adapter_status").mcp_policy_stub_reason(tool_name)


def _get_mcp_client_manager():
    return _tools_mcp_module("client_manager").get_mcp_client_manager()
...
def _evidence_planning_tool_names() -> tuple[str, ...]:
    return tuple(_module("app.orchestration.state_policy").EVIDENCE_PLANNING_TOOL_NAMES)


def _claim_tool_policy():
    return _module("app.planning.claim_tool_policy")


def _ticket_lookup_helpers():
    return _module("app.planning.ticket_lookup_helpers")


EVIDENCE_PLANNING_TOOL_NAMES = _evidence_planning_tool_names()
...
class ToolWhitelistBuilder:
    ...
    def __init__(
        ...
        self.capability_registry = capability_registry or _capability_registry_class()()
    ...
    def build_gap_whitelist(self, gap) -> ToolWhitelist:
        gap_tools_for_claim = _module("app.planning.claim_gap_fill_planner").gap_tools_for_claim
        claim_tool_policy = _claim_tool_policy()
        filter_allowed_tools = claim_tool_policy.filter_allowed_tools
        forbidden_tools_for_claims = claim_tool_policy.forbidden_tools_for_claims
        TICKET_GAP_FILL_TOOLS = _ticket_lookup_helpers().TICKET_GAP_FILL_TOOLS
        ...
        tried = {_resolve_tool_name(t) for t in (gap.already_tried_tools or [])}

        is_ticket_claim_type = _module("app.orchestration.fact_lookup_policy").is_ticket_claim_type
        ...
        if is_ticket_claim_type(gap.claim_type) or gap.claim_type == "opening_hours":
            ...
            pool = [_resolve_tool_name(t) for t in TICKET_GAP_FILL_TOOLS]
        else:
            pool = [
                _resolve_tool_name(t)
                for t in gap.suggested_tools
                if _resolve_tool_name(t) not in forbidden
            ]
    ...
    def _build_from_contract(self, state: TravelAgentState, prompt_context: dict | None = None) -> ToolWhitelist:
        ...
        if has_ticket and not elevation_only:
            candidates |= set(_ticket_lookup_helpers().TICKET_BOOKING_PRIMARY_TOOLS)
    ...
    def _build_legacy(self, state: TravelAgentState, prompt_context: dict | None = None) -> ToolWhitelist:
        ...
        if hard_needs and frame and (frame.requires_exact_fact or frame.requires_live_data):
            for need in hard_needs:
                candidates.update(_need_tool_profiles().get(need, []))
            if frame.query_scope == QueryScope.PLACE:
                candidates.update(_PLACE_VALIDATION_TOOLS)
        else:
            for need in needs:
                profile = _need_tool_profiles().get(need, [])
        ...
        if frame and frame.decision_type == DecisionType.BEST_TIME_TO_VISIT:
            candidates.update(_need_tool_profiles().get("best_time_to_visit", []))

        if frame and frame.decision_type == DecisionType.GENERAL_ADVICE:
            candidates.update(_need_tool_profiles().get("general_travel_advice", []))
        ...
        *,
        ...
        for tool_name in sorted(candidates, key=_tool_sort_key):
            ...
            if tool_name == "knowledge_prior":
                for need in needs:
                    if not _evidence_policy_class().model_prior_allowed_for(need):
                        restrictions.append(f"model_prior not allowed for need: {need}")

            agent_tool_catalog = _module("app.planning.agent_tool_catalog")
            enrich_descriptor_fields = agent_tool_catalog.enrich_descriptor_fields
            resolve_s5_task_class = agent_tool_catalog.resolve_s5_task_class
        ...
        for tool_name in EVIDENCE_PLANNING_TOOL_NAMES:
            ...
            if is_crowd_provider_tool(tool_name):
                ...
                if not provider_enabled_for_tool(tool_name, settings):
                    ...
                    blocked[tool_name] = "not_relevant_for_domain"
                continue
            if _is_mcp_policy_placeholder(_resolve_tool_name(tool_name)):
    ...
    def _is_configured(self, policy_tool_name: str) -> tuple[bool, str | None]:
        ...
        resolved = _resolve_tool_name(policy_tool_name)
        if is_ticket_provider_tool(resolved):
            settings = _settings()
            if not provider_enabled_for_tool(resolved, settings):
                return False, "disabled_by_config"
        ...
        if _is_mcp_policy_placeholder(_resolve_tool_name(policy_tool_name)):
            if not self._placeholder_config_enabled(policy_tool_name):
                return False, "disabled_by_config"
            return False, _mcp_policy_stub_reason(policy_tool_name) or "not_implemented"

        if _is_mcp_policy_tool(policy_tool_name) and _use_java_tool_gateway():
            return True, None
        ...
        if _is_mcp_policy_tool(policy_tool_name):
            settings = _settings()
            if not settings.mcp_enabled:
                return False, "MCP_ENABLED=false"

            stub_reason = _mcp_policy_stub_reason(policy_tool_name)
            if stub_reason:
