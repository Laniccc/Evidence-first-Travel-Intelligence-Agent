...
from app.evidence import claim_family_registry
...
class LookupClaim(BaseModel):
    ...
    def to_claim_requirement(self) -> Any:
        ClaimRequirement = importlib.import_module(
            "app.composition.response_contract"
        ).ClaimRequirement
        preferred_tools = claim_family_registry.preferred_tools_for_claim(self.claim_type)
    ...
    @classmethod
    def from_claim_requirement(cls, req: Any) -> "LookupClaim":
        family = req.claim_family or claim_family_registry.claim_family_for_type(req.claim_type)
        return cls(
