...
def is_fact_lookup_task(state: Any) -> bool:
    return _module("app.orchestration.fact_lookup_policy").is_fact_lookup_task(state)
...
def advance_entity_anchor_if_satisfied(state: Any) -> bool:
    return _lookup_chain().advance_entity_anchor_if_satisfied(state)


def ensure_lookup_chain_initialized(state: Any):
    return _lookup_chain().ensure_lookup_chain_initialized(state)


def get_lookup_chain(state: Any):
    return _lookup_chain().get_lookup_chain(state)


def mark_phase_complete(state: Any, phase: str) -> None:
    _lookup_chain().mark_phase_complete(state, phase)


def next_recommended_phase(state: Any):
    return _lookup_chain().next_recommended_phase(state)


def save_lookup_chain(state: Any, chain: Any) -> None:
    _lookup_chain().save_lookup_chain(state, chain)
...
def ticket_lookup_retrieval_complete(state: TravelAgentState) -> bool:
    return _module("app.execution.retrieval_attempt_ledger").retrieval_complete(
        state, "ticket_price"
    )
...
def filter_internal_policy_limitations(limitations: list[str]) -> list[str]:
    ...
    return _module("app.composition.response_sanitizer").sanitize_limitations(
        prelim, max_items=6
    )
...
def baidu_place_search_allowed_for_ticket(state: TravelAgentState) -> bool:
    ...
    ).lookup_entity_anchor_satisfied(state):
