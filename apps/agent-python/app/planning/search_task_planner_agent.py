...
from app.understanding.semantic_frame_model import SemanticEntities, SemanticFrame

from .claim_search_planner import ClaimSearchPlanner
from .search_task import SearchTask, normalize_tool_parameters
...
def _search_query_rewriter_class():
    return _module("app.planning.search_query_rewriter").SearchQueryRewriter
...
def _keyword_search_agent_class():
    return _module("app.execution.keyword_search_agent").KeywordSearchAgent


def _build_comparison_search_query(*args, **kwargs):
...
class SearchTaskPlannerAgent:
    ...
    async def run(self, state: Any, *, refine: bool = False) -> list[SearchTask]:
        ...
        max_tasks = min(3, cap)
        rewriter = _search_query_rewriter_class().from_planning_context(ctx, state)
    ...
    def _tasks_from_payload(
        ...
        *,
        ...
        for item in bucket:
            ...
            try:
                _keyword_search_agent_class().validate_task(task)
...
def _coerce_planner_task(
    ...
    if ctx.get("comparison_mode") and ctx.get("comparison_active_place"):
        ...
        query = _build_comparison_search_query(
            place,
            information_need,
            frame_stub,
