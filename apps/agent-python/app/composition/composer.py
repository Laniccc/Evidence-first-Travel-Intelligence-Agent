...
from app.composition._legacy_boundary import (
    final_attr,
    legacy_integration_attr,
)
from app.evidence.evidence_model import ClaimType, Evidence
from app.evidence.place_factsheet import PlaceFactSheet
from app.evidence.review import ReviewAspectName, ReviewAspectResult
from app.composition.itinerary import ItineraryItem, ItineraryPlan
from app.composition.response_models import ComparisonRow, RecommendationResult

CULTURAL_SETS = legacy_integration_attr("catalog", "destination_catalog", "CULTURAL_SETS")
ClaimSearchPlanner = final_attr("app.planning.claim_search_planner", "ClaimSearchPlanner")
is_search_miss_value = final_attr("app.planning.claim_search_planner", "is_search_miss_value")
resolve_start_location = legacy_integration_attr(
    "catalog",
    "location_resolver",
    "resolve_start_location",
)
get_place_catalog = legacy_integration_attr("catalog", "place_catalog", "get_place_catalog")
TravelTaskType = final_attr("app.understanding.travel_task", "TravelTaskType")
...
class ItineraryAgent:
    ...
    @classmethod
    def build(cls, goal: Any) -> ItineraryPlan:
        catalog = get_place_catalog()
