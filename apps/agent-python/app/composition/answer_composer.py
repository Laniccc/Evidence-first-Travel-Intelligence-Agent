...
from app.evidence.citation_policy import CitationPolicy
...
places_match = final_attr("app.planning.comparison_helpers", "places_match")
summarize_comparison_claims_for_compose = final_attr(
    "app.planning.comparison_helpers",
    "summarize_comparison_claims_for_compose",
)
TraceRecorder = final_attr("app.observability.trace", "TraceRecorder")
AnswerStyle = final_attr("app.understanding.intent_profile", "AnswerStyle")
UserNeedResidual = final_attr("app.planning.user_need_residual", "UserNeedResidual")
TravelAgentState = Any
normalize_llm_json_text = legacy_attr(".".join(["app", "utils", "llm_json"]), "normalize_llm_json_text")
...
class AnswerComposerAgent:
    ...
    def _nearby_guided_bundle_fields(
        ...
        if compose_mode != "nearby_guided" and not arguments.get("nearby_guided_presentation"):
            return {}
        from app.composition.nearby_guided_composition import build_nearby_guided_presentation
    ...
    async def _compose_nearby_guided(
        ...
        from app.composition.nearby_guided_composition import build_nearby_guided_draft
    ...
    def _fact_lookup_guided_bundle_fields(
        ...
        if compose_mode != "fact_lookup_guided" and not arguments.get("fact_lookup_presentation"):
            return {}
        from app.composition.fact_lookup_guided_composition import build_fact_lookup_presentation
    ...
    async def _compose_fact_lookup_guided(
        ...
        from app.composition.fact_lookup_guided_composition import build_fact_lookup_draft
    ...
    def _disambiguation_bundle_fields(
        ...
        if compose_mode != "place_disambiguation" and not arguments.get("disambiguation_presentation"):
            return {}
        from app.composition.place_disambiguation_composition import build_disambiguation_presentation
    ...
    async def _compose_place_disambiguation(
        ...
        from app.composition.place_disambiguation_composition import build_disambiguation_draft
    ...
    def _normalize_draft_payload(data: dict) -> dict:
        ...
        from app.composition.response_sanitizer import sanitize_limitations
    ...
    def _evidence_fallback_draft(bundle: dict) -> FinalAnswerDraft:
        ...
        if compose_mode == "nearby_guided":
            from app.composition.nearby_guided_composition import build_nearby_guided_draft_from_bundle
