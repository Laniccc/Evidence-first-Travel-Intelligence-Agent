...
from app.composition._legacy_boundary import (
    final_attr,
    legacy_attr,
)
from app.evidence.claim_policy_registry import enrich_claim_requirement
from app.evidence.evidence_policy import EvidencePolicy
from app.composition.response_contract import (
    ClaimRequirement,
    ClarificationPolicy,
    CompositionPolicy,
    EntityPolicy,
...
get_settings = legacy_attr(".".join(["app", "config"]), "get_settings")
_information_need_aliases = importlib.import_module("app.understanding.information_need_aliases")
infer_all_nearby_needs_from_text = _information_need_aliases.infer_all_nearby_needs_from_text
infer_nearby_need_from_text = _information_need_aliases.infer_nearby_need_from_text
is_nearby_need = _information_need_aliases.is_nearby_need
nearby_claims_for_retrieval = _information_need_aliases.nearby_claims_for_retrieval
normalize_information_needs = _information_need_aliases.normalize_information_needs
normalize_need = _information_need_aliases.normalize_need
resolve_nearby_need = _information_need_aliases.resolve_nearby_need
_lookup_need_aliases = importlib.import_module("app.understanding.lookup_need_aliases")
infer_lookup_needs_from_intent_subtypes = _lookup_need_aliases.infer_lookup_needs_from_intent_subtypes
infer_lookup_needs_from_text = _lookup_need_aliases.infer_lookup_needs_from_text
is_elevation_lookup_text = _lookup_need_aliases.is_elevation_lookup_text
resolve_lookup_need = _lookup_need_aliases.resolve_lookup_need
NormalizedUserRequest = final_attr(
    "app.understanding.normalized_user_request",
    "NormalizedUserRequest",
)
AnswerStyle = final_attr("app.understanding.intent_profile", "AnswerStyle")
EvidenceSensitivity = final_attr("app.understanding.intent_profile", "EvidenceSensitivity")
IntentProfile = final_attr("app.understanding.intent_profile", "IntentProfile")
PrimaryIntent = final_attr("app.understanding.intent_profile", "PrimaryIntent")
DecisionType = final_attr("app.understanding.semantic_frame_model", "DecisionType")
SemanticFrame = final_attr("app.understanding.semantic_frame_model", "SemanticFrame")
...
class ResponseContractCompiler:
    ...
    def compile(
        ...
        *,
        ...
        compile_lookup_claims = final_attr(
            "app.planning.lookup_claim_compiler",
            "compile_lookup_claims",
        )
        merge_lookup_claims_into_requirements = final_attr(
            "app.planning.lookup_claim_compiler",
            "merge_lookup_claims_into_requirements",
        )
