...
from app.composition._legacy_boundary import final_attr
...
from app.evidence.ticket_area_policy import build_ticket_area_policy, ticket_fact_scope_label
from app.evidence.opening_hours_extractor import (
    extract_opening_hours_from_evidence as evidence_extract_opening_hours_from_evidence,
)
from app.evidence.peak_elevation_extraction import (
    extract_peak_elevation_table as evidence_extract_peak_elevation_table,
)
from app.evidence.ticket_price_audit import rank_ticket_fact
from app.evidence.ticket_price_extractor import extract_ticket_price_from_evidence

TravelAgentState = Any
_fact_lookup_anchor_policy = importlib.import_module(
    "app.orchestration.fact_lookup_anchor_policy"
)
place_scope_note = _fact_lookup_anchor_policy.place_scope_note
resolved_place_label = _fact_lookup_anchor_policy.resolved_place_label
_fact_lookup_policy = importlib.import_module("app.orchestration.fact_lookup_policy")
collect_fact_clues = _fact_lookup_policy.collect_fact_clues
fact_need_label = _fact_lookup_policy.fact_need_label
has_official_fact_evidence = _fact_lookup_policy.has_official_fact_evidence
is_geographic_fact_need = _fact_lookup_policy.is_geographic_fact_need
primary_fact_need_from_state = _fact_lookup_policy.primary_fact_need_from_state
extract_place_candidates = final_attr(
    "app.orchestration.place_disambiguation",
    "extract_place_candidates",
)


def _ticket_product_policy():
    return importlib.import_module("app.planning.ticket_product_policy")


def _ticket_lookup_policy():
    return importlib.import_module("app.planning.ticket_lookup_policy")
...
def build_fact_lookup_presentation(state: TravelAgentState) -> dict:
    ...
    if is_geographic_fact_need(need):
        extract_peak_elevation_table = evidence_extract_peak_elevation_table
    ...
    get_lookup_claims_from_state = final_attr(
        "app.planning.lookup_claim_compiler",
        "get_lookup_claims_from_state",
    )
    ...
    if need == "opening_hours":
        extract_opening_hours_from_evidence = evidence_extract_opening_hours_from_evidence
    ...
    if need == "ticket_price":
        product_ctx = _ticket_product_policy().ensure_ticket_product_context(state)
...
def build_fact_lookup_draft(
    ...
    limitations.extend(
        _ticket_lookup_policy().filter_user_visible_limitations(list(state.limitations or []))
    )
    cited: list[str] = []
    ...
    limitations = _ticket_lookup_policy().filter_ticket_price_limitations(
        limitations, need=need
    )
