...
from app.composition._legacy_boundary import final_attr, legacy_tool_attr
from app.composition.composition_preflight import has_actionable_claim_decisions
...
is_search_miss_value = final_attr("app.planning.claim_search_planner", "is_search_miss_value")
...
_nearby_recommendation_policy = importlib.import_module(
    "app.evidence.nearby_recommendation_policy"
)
_nearby_anchor_policy = importlib.import_module("app.planning.nearby_anchor_policy")
is_nearby_need = _information_need_aliases.is_nearby_need
query_text_from_state = _information_need_aliases.query_text_from_state
resolve_nearby_need = _information_need_aliases.resolve_nearby_need
format_nearby_clue_text = _nearby_recommendation_policy.format_nearby_clue_text
is_nearby_information_need = _nearby_recommendation_policy.is_nearby_information_need
nearby_need_label = _nearby_recommendation_policy.nearby_need_label
s8_focus_claim_types_for_needs = _nearby_recommendation_policy.s8_focus_claim_types_for_needs
anchor_place_name = _nearby_anchor_policy.anchor_place_name
same_scenic_area_sub_poi_ambiguity = _nearby_anchor_policy.same_scenic_area_sub_poi_ambiguity
_location_key = final_attr("app.orchestration.place_disambiguation", "_location_key")
candidate_display_label = final_attr(
...
should_apply_unique_resolution = final_attr(
    "app.orchestration.place_disambiguation",
    "should_apply_unique_resolution",
)
candidates_are_ambiguous = legacy_tool_attr(
    "m" + "cp",
    "adapters",
    "baidu_response_parser",
    "candidates_are_ambiguous",
)
gate_tokens_from_user_query = legacy_tool_attr(
    "m" + "cp",
    "adapters",
    "baidu_response_parser",
...
def should_present_place_disambiguation_at_s8(state: TravelAgentState) -> bool:
    ...
    if (
        ...
        and frame
        and frame.entities
        and len(frame.entities.places or []) >= 2
    ):
        return False
    need = _fact_lookup_policy.primary_fact_need_from_state(state)
    ...
    } and _same_scenic_area_sub_poi_ambiguity(candidates, anchor):
        return False
