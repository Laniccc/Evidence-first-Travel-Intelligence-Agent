...
from app.composition.response_models import RecommendationResult
...
def _party_type():
    return importlib.import_module("app.understanding.user_query").PartyType
...
class TravelSuitabilityScorer:
    ...
    def _weights(goal: UserGoal) -> dict[str, float]:
        ...
        if _party_type().ELDERLY in goal.party:
            w["walking"] = 2.0
            w["crowd"] = 1.6
            w["transport"] = 1.5
        if _party_type().FAMILY in goal.party or _party_type().CHILDREN in goal.party:
            w["crowd"] = 1.5
            w["food"] = 1.4
        if _party_type().COUPLE in goal.party:
    ...
    def _audiences(fact_sheet: PlaceFactSheet, review: ReviewAspectResult, goal: UserGoal) -> tuple[list[str], list[str]]:
        ...
        if _party_type().COUPLE in goal.party and any(a.aspect == ReviewAspectName.PHOTO_EXPERIENCE for a in review.aspects):
    ...
    def _reasons(fact_sheet: PlaceFactSheet, review: ReviewAspectResult, goal: UserGoal) -> list[str]:
        ...
        if _party_type().ELDERLY in goal.party and (fact_sheet.walking_intensity or 0) > 0.6:
