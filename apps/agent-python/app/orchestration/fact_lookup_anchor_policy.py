...
def _place_disambiguation_guard() -> Any:
    return importlib.import_module("app.orchestration.place_disambiguation")


def extract_place_candidates(evidence: list) -> list[dict]:
    return _place_disambiguation_guard().extract_place_candidates(evidence)


def apply_unique_candidate(state: TravelAgentState, candidate: dict) -> TravelAgentState:
    return _place_disambiguation_guard().apply_unique_candidate(state, candidate)
