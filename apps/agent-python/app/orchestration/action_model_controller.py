...
from app.orchestration.s5_evidence_orchestrator import S5EvidenceOrchestratorAgent
from app.execution.actions import AgentAction, AgentActionType
from app.execution.s5_diversified_tool_selector import S5DiversifiedToolSelector
from app.planning.claim_search_planner import ClaimSearchPlanner
from app.planning.comparison_helpers import comparison_search_anchors, is_comparison_mode
from app.planning.ticket_lookup_helpers import collect_official_discovery_search_results
from app.planning.claim_gap_fill_planner import gap_tools_for_claim, order_gap_tools
from app.planning.claim_tool_policy import filter_allowed_tools
from app.orchestration._legacy_boundary import (
    legacy_attr,
    legacy_config_attr,
)
from app.orchestration.state_policy import StateNodePolicy
from app.orchestration.fact_lookup_policy import is_ticket_claim_type
from app.orchestration.place_disambiguation import (
    clear_disambiguation_pending,
    disambiguation_pending_without_city,
    next_disambiguation_branch,
    try_resolve_disambiguation,
)
from app.composition.composition_preflight import should_compose_over_clarification
from app.evidence.evidence_policy import EvidencePolicy
from app.evidence.evidence_brief import build_evidence_brief
from app.evidence.evidence_model import Evidence
from app.evidence.evidence_signal_utils import (
    any_contradiction_signal as evidence_any_contradiction_signal,
    is_day_trip_query as evidence_is_day_trip_query,
)
...
from app.planning.s5_information_domain import InformationDomain, ProviderGroup, S5ToolRole
from app.planning.agent_tool_catalog import route_tools_priority
from app.planning.tool_whitelist_model import ToolWhitelist

LLMClient = legacy_attr(".".join(["app", "integrations", "llm"]), "LLMClient")
NEED_TOOL_PROFILES = legacy_attr(
    ".".join(["app", "integrations", "mcp", "tool_specs"]),
    "NEED_TOOL_PROFILES",
)
is_ticket_provider_tool = legacy_attr(
    ".".join(["tools", "ticketing", "provider_config"]),
...
class ActionModelController:
    ...
    def _contradiction_decompose_due(self, state: TravelAgentState, prompt_context: dict) -> bool:
        any_contradiction_signal = evidence_any_contradiction_signal
    ...
    def _callable_non_search_queue(
        ...
        pick_baidu_uid_from_evidence = legacy_attr(
            ".".join(["tools", "mcp", "adapters", "baidu_response_parser"]),
    ...
    def _plan_gap_filling(
        ...
        from app.planning.evidence_gap_request import EvidenceGapRequest
        ...
        if search_idx >= 0 and search_idx < min(len(templates), search_budget):
            ...
            return AgentAction(
                ...
                arguments={
                    ...
                    "preferred_tool": "",
                    "tool_parameters": dict(gap.tool_parameters),
                },
    ...
    def _needs_coordinate_resolution(state: TravelAgentState) -> bool:
        resolve_coordinates_from_evidence = legacy_attr(
            ".".join(["tools", "mcp", "adapters", "baidu_response_parser"]),
    ...
    def _inject_route_tools_if_needed(
        ...
        is_day_trip_query = evidence_is_day_trip_query
    ...
    def _tool_arguments_for(tool: str, state: TravelAgentState, prompt_context: dict) -> dict:
        ...
        if tool == "official_source_discovery_mcp":
            ...
            if urls:
                args["urls"] = urls
            args["probe_top_n"] = 1
            resolve_official_whitelist_url = legacy_attr(
                ".".join(["tools", "official_source", "whitelist_resolver"]),
                "resolve_official_whitelist_url",
            )
