...
from app.orchestration.fact_lookup_policy import (
...
from app.planning.lookup_research_chain import (
    LookupPhase,
...
def next_recommended_phase(state: TravelAgentState) -> LookupPhase | None:
    ...
    for phase in order:
        ...
        if phase == "peak_elevation_lookup":
            from app.orchestration.fact_lookup_anchor_policy import resolved_place_label

            table = evidence_extract_peak_elevation_table(
                list(state.evidence or []),
                place_name=resolved_place_label(state),
            )
            if not peak_extraction.elevation_needs_peak_gap(table, exact_required=True):
...
def build_retrieval_audit(state: TravelAgentState) -> RetrievalAudit:
    ...
    if official and actionable:
        ...
        table = evidence_extract_peak_elevation_table(
            evidence, place_name=resolved_place_label(state)
        )
        structured = state.structured_result or {}
        if isinstance(structured.get("peak_elevation_table"), dict):
            table = peak_extraction.extract_peak_elevation_table(
                evidence, place_name=resolved_place_label(state)
            )
        if peak_extraction.elevation_needs_peak_gap(table, exact_required=True):
...
def advance_entity_anchor_if_satisfied(state: TravelAgentState) -> bool:
    ...
    from app.orchestration.lookup_entity_resolution_policy import lookup_entity_anchor_satisfied
...
def lookup_mandatory_entity_anchor(state: TravelAgentState, step: int) -> bool:
    from app.orchestration.lookup_entity_resolution_policy import (
