...
from app.evidence.claim_policy_registry import CLAIM_TYPE_ALIASES
from app.evidence.evidence_usage_role import (
    is_entity_anchor_only as evidence_is_entity_anchor_only,
)
from app.evidence.peak_elevation_extraction import (
    classify_elevation_text as evidence_classify_elevation_text,
)
from app.orchestration.fact_lookup_anchor_policy import (
    GEO_FACT_NEEDS,
    interpret_place_for_fact_need,
    is_geographic_fact_need,
    resolved_place_label,
)
from app.planning.lookup_claim_compiler import primary_lookup_claim
from app.understanding.information_need_aliases import query_text_from_state
...
from app.understanding.semantic_frame_model import DecisionType, TaskFamily
...
def is_authoritative_geo_evidence(ev: Evidence) -> bool:
    classify_elevation_text = evidence_classify_elevation_text
...
def collect_fact_clues(state: TravelAgentState, *, limit: int = 8) -> list[dict]:
    ...
    from app.orchestration.fact_lookup_anchor_policy import elevation_clue_rank
    ...
    for ev in state.evidence or []:
        if not isinstance(ev, Evidence):
            continue
        is_entity_anchor_only = evidence_is_entity_anchor_only

        if is_entity_anchor_only(ev, need):
            continue
        if is_ticket_claim_type(need):
            is_ticket_price_noise_evidence = importlib.import_module(
...
def pipeline_search_queries(state: TravelAgentState, need: str) -> list[str]:
    from app.orchestration.lookup_query_objectives import (
        build_lookup_query_objectives,
        objective_to_search_query,
    )
    from app.orchestration.lookup_research_chain import ensure_lookup_chain_initialized
    from app.planning.lookup_research_chain import SourceFamily
