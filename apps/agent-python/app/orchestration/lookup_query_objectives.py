...
from app.orchestration.fact_lookup_anchor_policy import raw_place_label, resolved_place_label
from app.orchestration.fact_lookup_policy import fact_need_label, is_geographic_fact_need
from app.orchestration.lookup_research_chain import ensure_lookup_chain_initialized, get_lookup_chain
from app.evidence.peak_elevation_extraction import (
    discover_peak_names_from_evidence as evidence_discover_peak_names_from_evidence,
)
from app.planning.lookup_research_chain import LookupQueryObjective, SourceFamily
...
def _ticket_multi_query_objectives(
    ...
    *,
    ...
    ticket_ladder = importlib.import_module("app.planning.ticket_price_query_ladder")
    ticket_policy = importlib.import_module("app.planning.ticket_product_policy")

    ticket_policy.ensure_ticket_product_context(state)
    place = resolved_place_label(state) or (anchors[0] if anchors else "")
    product_kws = ticket_policy.ticket_product_keywords(state)
    tier_by_family = {
    ...
    pairs = [
        (tier, q)
        for tier, q in ticket_ladder.build_ticket_price_escalation_queries(
            state, max_queries=max_objectives * 3
        )
...
def build_peak_elevation_objectives(
    ...
    *,
    ...
    if not names:
        names = evidence_discover_peak_names_from_evidence(list(state.evidence or []))
    objectives: list[LookupQueryObjective] = [
        LookupQueryObjective(
