...
from app.planning.information_need_planner import InformationNeedPlanner
from app.planning.comparison_helpers import (
    comparison_places_from_state,
    disambiguated_place_label,
    reset_per_place_search_state,
    stamp_evidence_place,
)
from app.planning.user_need_residual import attach_user_need_residual
from app.composition.composer import ItineraryAgent
from app.composition.response_models import StructuredResult
from app.understanding.intent_agent import IntentAgent, RegionGateAgent
...
from app.understanding.travel_task_to_user_goal_adapter import SUPPORTED_REGIONS, TravelTaskToUserGoalAdapter
from app.composition.response_contract_compiler import ResponseContractCompiler
from app.evidence.citation_checker import CitationChecker
from app.orchestration.travel_agent_state import ConflictRecord, TravelAgentState
from app.orchestration.agent_core_control_tools import AgentCoreControlTools
from app.orchestration.agent_core_store import ensure_agent_core_store, project_agent_core
from app.orchestration.agent_core_supervisor import RootAgentSupervisor
from app.orchestration.fact_lookup_task_orchestration import resolve_fact_lookup_compose_mode
from app.planning.nearby_task_orchestration import resolve_nearby_compose_mode
from app.orchestration._legacy_boundary import legacy_attr, legacy_config_attr
...
from app.context.conversation_context import ConversationMemory
from app.evidence.evidence_model import Evidence
from app.evidence.place_factsheet import PlaceFactSheet
from app.evidence.review import ReviewAspectResult
from app.evidence.review_mining_agent import VerifierAgent
from app.planning.place_research_agent import PlaceResearchAgent
from app.planning.agent_tool_catalog import resolve_s5_task_class
...
from app.contracts.response import TravelQueryResponse
from app.tools import ToolRegistry
from app.tools.tool_router import ToolRouter
from app.understanding.intent_profile import EvidenceSensitivity, PrimaryIntent
from app.understanding.semantic_frame_model import AnswerMode, DecisionType, TaskFamily
from app.understanding.travel_task import PlaceContext, TravelTaskType
from app.understanding.user_query import IntentType, RegionGateResult, UserContext, UserGoal

get_settings = legacy_config_attr("get_settings")
resolve_city_country_from_text = legacy_attr(
    ".".join(["app", "integrations", "catalog", "location_resolver"]),
    "resolve_city_country_from_text",
)
get_place_catalog = legacy_attr(
    ".".join(["app", "integrations", "catalog", "place_catalog"]),
    "get_place_catalog",
)
LLMClient = legacy_attr(".".join(["app", "integrations", "llm"]), "LLMClient")
...
class TravelAgentStateMachine:
    ...
    def _select_highest_priority_gap(state: TravelAgentState) -> EvidenceGapRequest | None:
        ...
        gap_rows = getattr(report, "evidence_gap_" + "re" + "quests", []) if report else []
        if not gap_rows:
            return None
    ...
    def _resolve_compose_mode(state: TravelAgentState) -> str:
        from app.composition.place_disambiguation_composition import should_present_place_disambiguation_at_s8
        ...
        try:
            ...
            if state.evidence_decision_report:
                ...
                gaps = [
                    ...
                    for g in getattr(
                        state.evidence_decision_report,
                        "evidence_gap_" + "re" + "quests",
                    )
                ]
    ...
    def _to_response(self, state: TravelAgentState, confidence: float) -> TravelQueryResponse:
        from app.composition.response_sanitizer import sanitize_answer_text, sanitize_limitations
