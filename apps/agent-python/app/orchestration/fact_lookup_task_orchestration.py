...
def fact_s5_may_finish_early(state: TravelAgentState, step: int) -> bool:
    ...
    if need == "ticket_price":
        ...
        ticket_lookup_has_price_evidence = importlib.import_module(
            "app.execution.ticket_lookup_attempt_tracker"
...
def fact_s5_system_append(state: TravelAgentState) -> str:
    if not is_fact_lookup_task(state):
        return ""
    from app.orchestration.fact_lookup_anchor_policy import place_scope_note, resolved_place_label
...
def fact_s5_planning_context(state: TravelAgentState) -> dict:
    if not is_fact_lookup_task(state):
        return {}
    from app.orchestration.fact_lookup_anchor_policy import place_scope_note, raw_place_label, resolved_place_label
