...
from app.execution.retrieval_attempt_ledger import retrieval_complete
from app.execution.s5_diversified_tool_selector import select_tool_for_subagent
from app.execution.ticket_lookup_attempt_tracker import (
    normalize_subagent_objective,
    record_subagent_objective,
    subagent_objective_seen,
    ticket_lookup_has_price_evidence,
)
from app.evidence.evidence_signal_utils import is_day_trip_query as evidence_is_day_trip_query
from app.orchestration._legacy_boundary import legacy_attr, legacy_config_attr
from app.orchestration.s5_subagent_registry import ORCHESTRATOR_SUBAGENT_NAMES, subagent_definitions_for_prompt
from app.orchestration.travel_agent_state import TravelAgentState
from app.orchestration.fact_lookup_policy import is_fact_lookup_task, primary_fact_need_from_state
from app.orchestration.fact_lookup_task_orchestration import (
    fact_s5_may_finish_early,
    fact_s5_planning_context,
    fact_s5_skip_fact_search,
    fact_s5_system_append,
)
from app.orchestration.lookup_entity_resolution_policy import entity_resolution_allowed_for_lookup
from app.orchestration.lookup_query_objectives import build_lookup_query_objectives
from app.orchestration.lookup_research_chain import (
...
from app.orchestration.place_disambiguation import disambiguation_pending_without_city
from app.planning.agent_core_prompt_guidance import agent_core_task_guidance_block
...
from app.planning.search_query_rewriter import SearchQueryRewriter
from app.planning.nearby_task_orchestration import (
...
from app.planning.s5_poi_anchor_policy import (
    build_entity_resolution_arguments,
    mandatory_poi_entity_required,
)
from app.planning.ticket_product_policy import ensure_ticket_product_context
from app.understanding.information_need_aliases import nearby_needs_set
...
def _route_endpoints_from_text(text: str) -> tuple[str | None, str | None]:
    parser = legacy_attr(
        ".".join(["app", "integrations", "mcp", "tool_arguments"]),
        "_route_endpoints_from_text",
    )
    return parser(text)
