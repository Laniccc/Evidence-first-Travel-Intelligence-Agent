...
from app.evidence.geo_fact_gazetteer import supplement_elevation_from_gazetteer
...
from app.execution.s5_diversified_tool_selector import (
...
from app.orchestration.agent_core_pipeline_gate import PipelineGate, ToolVisibility
from app.orchestration.agent_core_store import ensure_agent_core_store, project_agent_core
from app.orchestration.fact_lookup_task_orchestration import fact_s5_planning_context
from app.orchestration.lookup_research_chain import build_lookup_research_context
from app.orchestration.s5_subagent_registry import subagent_definitions_for_prompt
...
from app.planning.comparison_helpers import comparison_max_tool_calls, is_comparison_mode
from app.planning.information_need_planner import InformationNeedPlanner
from app.planning.nearby_task_orchestration import nearby_s5_planning_context
from app.planning.ticket_lookup_policy import apply_ticket_gap_phase_override
...
ensure_http_mcp_services = legacy_attr(
    ".".join(["tools", "m" + "cp", "http_autostart"]),
    "ensure_http_mcp_services",
)
TravelAgentState = Any


def mcp_tool_invocation_ready(*args, **kwargs) -> bool:
    readiness_check = legacy_attr(
        ".".join(["app", "integrations", "mcp", "tool_arguments"]),
        "mcp_tool_invocation_ready",
    )
    return readiness_check(*args, **kwargs)
