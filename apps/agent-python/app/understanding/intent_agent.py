...
def _location_resolver():
    return importlib.import_module("app.integrations.catalog.location_resolver")


def iter_city_country():
    return _location_resolver().iter_city_country()


def iter_location_aliases():
    return _location_resolver().iter_location_aliases()


def resolve_start_location(value):
    return _location_resolver().resolve_start_location(value)


def get_place_catalog():
    return importlib.import_module("app.integrations.catalog.place_catalog").get_place_catalog()


class RegionGateAgent:
    COUNTRY_KEYWORDS = {
        "Japan": ["日本", "japan", "东京", "京都", "大阪", "奈良", "札幌", "冈山", "冲绳", "箱根", "新宿", "清水寺", "伏见", "岚山"],
        "China": ["中国", "china", "北京", "上海", "杭州", "苏州", "西安", "成都", "故宫", "颐和园", "天坛", "禾木", "喀纳斯", "新疆", "阿勒泰"],
