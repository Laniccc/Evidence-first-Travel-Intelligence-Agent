...
from app.context import ConversationContext, UserProfile
...
from .travel_task import PlaceContext, TravelTask, TravelTaskType
from .user_query import PartyType, PaceType, UserContext


def resolve_city_country_from_text(text: str):
    return importlib.import_module("app.integrations.catalog.location_resolver").resolve_city_country_from_text(text)


def infer_nearby_need_from_text(text: str) -> str | None:
    return importlib.import_module("app.understanding.information_need_aliases").infer_nearby_need_from_text(text)


def _place_resolver_class():
    return importlib.import_module("app.integrations.places.place_resolver").PlaceResolver


class PlaceResolver:
    @staticmethod
    def resolve_sync(*args, **kwargs):
        return _place_resolver_class().resolve_sync(*args, **kwargs)
